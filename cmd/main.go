...
// Package main is the entry point for the barbican-operator.
package main
...
import (
	...
	"github.com/openstack-k8s-operators/barbican-operator/internal/controller"
	webhookv1beta1 "github.com/openstack-k8s-operators/barbican-operator/internal/webhook/v1beta1"
...
func main() {
	...
	if err := (&controller.BarbicanAPIReconciler{
	...
	if err := (&controller.BarbicanWorkerReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&controller.BarbicanKeystoneListenerReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupBarbicanWebhookWithManager(mgr); err != nil {
