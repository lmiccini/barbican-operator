...
import (
	...
	"github.com/openstack-k8s-operators/barbican-operator/internal/barbicanworker"
...
// generateServiceConfigs creates the Secret holding the service configuration.
// It returns the parent Barbican CR's DefaultConfigOverwrite map so the caller
// can pass it to collectOverwriteKeys without a redundant API fetch.
func (r *BarbicanWorkerReconciler) generateServiceConfigs(
	...
	// Keys matching *.conf (CustomServiceConfigFileName, DefaultsConfigFileName, etc.)
	// are placed into /etc/barbican/barbican.conf.d/ for oslo.config snippet merging.
	// All other keys (from DefaultConfigOverwrite, e.g. policy.yaml) are placed into
	// /etc/barbican/ as standalone files via SubPath mounts.
	...
	if owner != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to fetch parent Barbican %s/%s: %w", instance.Namespace, owner, err)
		}
		parentOverwrite = ownerInstance.Spec.DefaultConfigOverwrite
		maps.Copy(customData, parentOverwrite)
	}
...
func (r *BarbicanWorkerReconciler) reconcileNormal(ctx context.Context, instance *barbicanv1beta1.BarbicanWorker, helper *helper.Helper) (ctrl.Result, error) {
	...
	if instance.Spec.NotificationsURLSecret != "" {
		Log.Info(fmt.Sprintf("[Worker] Verify secret '%s'", instance.Spec.NotificationsURLSecret))
	...
	//
	parentOverwrite, err := r.generateServiceConfigs(ctx, helper, instance, &configVars)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
	...
	overwriteKeys := collectOverwriteKeys(parentOverwrite, instance.Spec.DefaultConfigOverwrite)

	Log.Info(fmt.Sprintf("[Worker] Defining deployment '%s'", instance.Name))
	// Define a new Deployment object
	deplDef := barbicanworker.Deployment(instance, inputHash, serviceLabels, serviceAnnotations, topology, overwriteKeys)
